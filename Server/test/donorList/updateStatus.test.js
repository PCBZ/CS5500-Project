...
describe('Update List Status API Tests', () => {
  let testUser;
  let testEvent;
  let testList;
  let authToken;
  ...
  it('should update list status to completed', async () => {
    ...
    expect(response.body).toHaveProperty('message', 'List status updated successfully');
    expect(response.body.list).toHaveProperty('review_status', 'completed');

    // Verify status has been updated in database
    const updatedList = await prisma.eventDonorList.findUnique({
      where: { id: testList.id }
    });
    expect(updatedList.reviewStatus).toBe('completed');
  });
